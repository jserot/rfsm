...
let type_product = function
  | [t] -> t
  | ts -> TyProduct ts
let ty_int () = type_constr "int" [] None
let ty_bool () = type_constr "bool" [] None
...
let copy_type tvbs ty =
  let rec copy ty = 
    ...
    | TyConstr (c, args, sz) ->
       TyConstr (c, List.map copy args, sz)
  in
  copy ty
...
let rec pp_typ fmt t =
  ...
  | TyConstr (c,ts,sz) -> fprintf fmt " %a %s%a" (Msic.Misc.pp_list_h pp_typ) ts c pp_sz sz
  | TyProduct [t] -> pp_typ fmt t
  | TyProduct ts -> Msic.Misc.pp_list_h ~sep:"*" pp_typ fmt ts
  | TyArrow (t1,t2) -> fprintf fmt "%a -> %a" pp_typ t1 pp_typ t2 
  | TyVar v -> fprintf fmt "%a" pp_tvar v  (* TODO: use alphabetic names *)

and pp_tvar fmt tv = Format.fprintf fmt "_%d" tv.stamp (* TO FIX *) 
...
let pp_typ_scheme fmt t =
  ...
  | params ->
     fprintf fmt "@[<h>forall %a. %a@]" (Msic.Misc.pp_list_h ~sep:"," pp_tvar) t.ts_params pp_typ t.ts_body
