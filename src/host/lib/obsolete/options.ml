...
let options_spec = [
"-main", Arg.String (Options.set_main_prefix), "set prefix for the generated main files";
"-dump_static", Arg.Unit (Options.set_dump_static), "dump static representation of model(s)/program to stdout";
"-target_dir", Arg.String (Options.set_target_dir), "set target directory (default: .)";
"-dot", Arg.Unit (Options.set_dot), "generate .dot representation of model(s)/program";
...
"-dot_actions_nl", Arg.Unit (Options.set_dot_actions_nl), "write actions with with a separating newline";
"-trace", Arg.Int (Options.set_sim_trace), "set trace level for simulation (default: 0)";
"-synchronous_actions", Arg.Unit (Options.set_synchronous_actions), "interpret actions synchronously";
"-normalize", Arg.Unit (Options.set_normalize), "move output assignations from states to transitions";
];
