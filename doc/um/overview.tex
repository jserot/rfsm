...
\step The operations on values of type \texttt{int} are : "\verb|+|", "\verb|-|", "\verb|*|",
"\verb|/|" and "\verb|%|" (modulo).
...
\step Indexing can also be used to access a given bit within an integer value. For example, if
\verb|x| is an integer ranging from 0 to 255 then \verb|x[i]|, where \verb|i| ranges from 0 to 7
gives access to the \verb|i|$^{th}$ bit of \verb|x| (with index 0 giving the LSB and index 7 the
MSB). Integer bits can be both read and written. The behavior is unspecified if the index falls
