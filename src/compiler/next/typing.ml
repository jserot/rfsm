...
let type_type_decl tenv { td_desc = d; td_loc = loc } =
  let open Typing in
  match d with
  | TD_Alias (id, te) ->
...
let type_cst_decl tenv { cst_desc = cd } =
...
let type_program p = 
  let tenv =
       Typing.builtin_tenv
    |> Misc.left_fold type_type_decl p.p_type_decls
    |> Misc.left_fold type_cst_decl p.p_cst_decls
    |> Misc.left_fold type_fn_decl p.p_fn_decls
    |> Misc.left_fold type_fsm_decl p.p_fsm_models in
    |> Misc.left_fold type_global p.p_globals in
  p, tenv
