...
let add_param env _ = env (* No parameter in the [szvars] guest language *)
...
and size_of_size_expr c sz = 
  ...
  | _, _ -> Rfsm.Misc.fatal_error "Guest.Typing.size_of_size_expr" (* Should not happen thx to parsing defns *)
...
let rec type_expression env e =
  ...
  let ty = match e.Annot.desc with
    ...
    | Syntax.ERanged (a,i1,i2) ->
       let r = type_ranged_expr ~loc:e.Annot.loc env a i1 i2 (* shared with type_lval *) in
       r
...
and type_ranged_expr ~loc env a i1 i2 = 
  ...
  | TyConstr("int",_,_) ->
     let sz = 
      ...
      | Syntax.EInt hi, Syntax.EInt lo -> Types.Sz1 (hi-lo+1) (* The result size can here be statically determined ... *)
