...
Listing~\ref{lst:rfsm-mouse} is an example of a simple RFSM program\footnote{This program is
...
\verb|k|. The \emph{transitions} of this FSM are given after the \verb|trans:| keyword in the form :
\begin{center}
...
\medskip
The third part of the program (lines 17--19) lists \textbf{global inputs and outputs}\footnote{And
  also, in case of multi-FSM programs, \emph{shared} events and variables. See
  Sec.~\ref{sec:globals}.}. 
For global outputs the declaration simply gives a name and a type.
For global inputs, the declaration also specifies the \textbf{stimuli} which are attached to the
corresponding input for simulating the system. The program of Listing~\ref{lst:rfsm-gensig} uses two
kinds of stimuli\footnote{See Sec.~\ref{sec:globals} for a complete description of stimuli.}. The
stimuli attached to input \verb|H| are declared as \emph{periodic}, with a period of 10 time units,
a start time of 0 and a end time of 80. This means than an event will be produced on
this input at time 0, 10, 20, 30, 40, 50, 60, 70 and 80. The stimuli attached to input \verb|E| say
that this input will respectively take value 0, 1 and 0 at time 0, 25 and 35 (thus producing a
``pulse'' of duration 10 time units starting at time 25).
...
section. This presentation is deliberately informal. A formal description of the language syntax can
be found in Appendix~A.  Future versions of this document will also provide a description of the
formal semantics of the underlying models.
...
\medskip
\textbf{Builtin types} are : \texttt{bool}, \texttt{int} and \texttt{event}.
...
value of variable \verb|ctr| is inferior to 8. 
...
\begin{center}
\framebox{\lstinline[language=Rfsm]'output name : type'}
\end{center}
...
\begin{center}
\framebox{\lstinline[language=Rfsm]'input name : type = stimuli'}
\end{center}
...
Shared objects are used to represent inter-connections between FSM instances. This situation only
...
\begin{center}
\framebox{\lstinline[language=Rfsm]'shared name : type'}
\end{center}
...
results are illustrated in Fig~\pageref{fig:rfsm-cntmod8-vcd}. 
...
This document deliberately focuses on syntax. Formalizing the semantics of programs made of
...
both the transitions at line 10 and 11 are enabled. In fact, here's what we get when we try to
simulate the above program :
...
Of course, this can avoided by modifying the stimuli attached to input \verb|StartStop| so that the
corresponding events are never emitted at time $t=n\times 10$. But this is, in a sence, cheating,
since this event is supposed to modelize user interaction which occurs, by essence, at impredictible
dates. 
...
case of the example above, this approach gives the following modified program, in which the
\verb|startstop| event is given precedence against the \verb|h| event when the model is in state
\verb|Running|~:
...
\medskip
A sequential interpretation naturally fits software execution models, in which FSM variables are
