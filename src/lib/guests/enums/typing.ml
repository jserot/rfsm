...
let mk_type_constr0 name = Types.TyConstr (name, [], None)

let is_type_constr0 name ty = match ty with
  | Types.TyConstr (name', [], _) when name=name' -> true
  | _ -> false
...
let rec type_expression env e =
  let ty = match e.Annot.desc with
    ...
    | Syntax.EArr (a,i) -> type_array_access ~loc:e.Annot.loc env a i 
...
and type_array_access ~loc env a i =
  ...
  let ty_res =
    ...
    | _ -> raise (Types.Type_conflict (loc, ty_arg, Types.TyConstr ("array", [], None)))
    end in
  ty_res
