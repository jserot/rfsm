...
let pp_type_expr fmt te = 
  ...
  | Some t -> pp_typ fmt t 
  | None -> Rfsm.Misc.fatal_error "Full.Systemc.pp_type_expr"
...
let pp_expr fmt e = 
  ...
  | Syntax.ECon0 c, Some (Types.TyConstr (t,_)) -> fprintf fmt "%s::%a" t pp_ident c
...
let pp_typed_symbol fmt (name,t) =
  ...
  | Some (Types.TyArray (t',sz)) -> fprintf fmt "%a %a[%d]" pp_typ t' pp_ident name sz
...
let pp_cst_decl fmt name t = 
  let open Types in 
  match t.Syntax.Annot.typ with
  | Some (TyArray _) -> fprintf fmt "extern %a" pp_typed_symbol (name,t)
