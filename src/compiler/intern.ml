...
let mk_fsm_inst models globals { fi_desc=f; fi_loc=loc } = 
  ...
  let iargs, oargs =
    try Misc.list_split_at (List.length model.Fsm.fm_inps) f.fi_args
    with Invalid_argument _ -> Error.fsm_mismatch "input/output(s)" loc f.fi_name in
  let mk_global id =
  ...
  Fsm.build_instance
    ...
    ~model:model
    ~params:params
    ~inps:(List.map mk_global iargs)
    ~outps:(List.map mk_global oargs)
