...
expr:
  ...
  | a = LID LBRACKET i=expr RBRACKET { mk ~loc:($symbolstartofs,$endofs) (EIndexed (a,i)) }
  ...
  | a = LID LBRACKET i1=expr COLON i2=expr RBRACKET { mk ~loc:($symbolstartofs,$endofs) (ERanged (a,i1,i2)) }
...
lhs:
  ...
  | id = LID LBRACKET idx = expr RBRACKET { mk ~loc:($symbolstartofs,$endofs) (LhsIndex (id, idx)) }
  | a=LID DOT f=LID { mk ~loc:($symbolstartofs,$endofs) (LhsRField (a, f)) }
  | a=LID LBRACKET hi=expr COLON lo=expr RBRACKET { mk ~loc:($symbolstartofs,$endofs) (LhsRange (a,hi,lo)) }
