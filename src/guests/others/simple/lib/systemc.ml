...
let pp_typ fmt t =
  ...
  let open Format in 
  match t with
    | TyConstr ("int",[]) -> fprintf fmt "int" 
    | TyConstr ("float",[]) -> fprintf fmt "%s" (if Rfsm.Systemc.cfg.Rfsm.Systemc.sc_double_float then "double" else "float")
...
let pp_type_expr fmt te = pp_typ fmt te.Rfsm.Annot.typ 
...
let pp_expr fmt e = 
  ...
  | Syntax.ECon0 c, Types.TyConstr (t,_) -> fprintf fmt "%s::%a" t pp_ident c
  ...
  | Syntax.ERecordExt fs, Types.TyRecord (name,_) ->
     fprintf fmt "%s(%a)" name (Rfsm.Ext.List.pp_h ~sep:"," (pp_rfield_value level)) fs 
...
let pp_value fmt v = 
  ...
  | Val_array vs -> fprintf fmt "{%a}" (Rfsm.Ext.List.pp_h ~sep:"," pp_v) (Array.to_list vs)
...
let pp_typed_symbol fmt (name,t) =
  ...
  | Types.TyArray (t',sz) -> fprintf fmt "%a %a[%d]" pp_typ t' pp_ident name sz
...
let pp_record_type_defn fmt (name,fields) = 
  ...
  fprintf fmt "class %a {\n" pp_ident name;
  fprintf fmt "public:\n";
  fprintf fmt "  struct { %a } repr;\n" (Rfsm.Ext.List.pp_h ~sep:" " pp_rfield) fields;
  fprintf fmt "  %a() { };\n" pp_ident name;
  fprintf fmt "  %a(%a) { %a };\n"
    pp_ident name
    (Rfsm.Ext.List.pp_h ~sep:"," pp_typed_name) fields
    (Rfsm.Ext.List.pp_h ~sep:" " (fun fmt (n,_) -> fprintf fmt "repr.%s = %s;" n n)) fields; 
  fprintf fmt "  inline bool friend operator == ( const %a& v1, const %a& v2) { return %a; }\n"
    pp_ident name
    pp_ident name
    (Rfsm.Ext.List.pp_h ~sep:" && " (fun fmt (n,_) -> fprintf fmt "v1.repr.%s==v2.repr.%s" n n)) fields;
  ...
  fprintf fmt "    os << \"{\" << %a << \"} \";\n"
    (Rfsm.Ext.List.pp_h ~sep:" << \",\" << " (fun fmt (n,_) -> fprintf fmt  "\"%s=\" << v.repr.%s" n n)) fields; 
...
let pp_enum_type_defn fmt (name,vs) = 
  ...
  fprintf fmt "  enum values { %a };\n"
    (Rfsm.Ext.List.pp_h ~sep:"," (fun fmt (v,i) -> fprintf fmt "%s=%d" v i)) (List.mapi (fun i v -> v,i) vs);
...
let rec pp_type_impl_desc fmt td =
  ...
  | Syntax.TD_Enum (name,ctors) ->
       Format.fprintf fmt "const char* %a::names[%d] = { %a };\n" 
         pp_ident name
         (List.length ctors)
         (Rfsm.Ext.List.pp_h ~sep:", " pp_ctor) ctors
    | _ -> ()
