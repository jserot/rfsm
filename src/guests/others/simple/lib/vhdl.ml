...
let rec vhdl_type_of t =
  ...
  match Types.real_type t with 
    ...
    | Types.TyConstr ("float",[]) -> Real
    | Types.TyConstr ("char",[]) -> Char
    | TyConstr ("int",[]) -> Integer None  (* Very crude translation here *)
    | TyConstr (c,[]) -> Enum (c,[]) 
...
let rec pp_value fmt (v,ty) =
  ...
  | Value.Val_fn _, _ -> Rfsm.Misc.not_implemented "VHDL: translation of function value"
  ...
  | _, _ ->
     Rfsm.Misc.fatal_error "Vhdl.pp_value"
